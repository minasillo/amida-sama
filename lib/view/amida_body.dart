...
class AmidaBody extends StatefulWidget {
  const AmidaBody({required this.participantList, required this.wininngImagePath, super.key});
...
class _AmidaBodyState extends State<AmidaBody> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _syncLotteryList();
    _horizontalLines = _generateRandomHorizontalLines(widget.participantList.length);
  ...
  void _syncLotteryList() {
    lotteryList = List.generate(
      ...
      (index) => _selectedWinningIndices.contains(index) ? AmidaLottery.win : AmidaLottery.lose,
    );
  }
  ...
  List<HorizontalLine> _generateRandomHorizontalLines(int columns) {
    ...
    for (var i = 0; i < columns - 1; i++) {
      ...
      prevYPositionFactors..clear()..addAll(tempYPositionFactors);
      horizontalLinesList.addAll(tempYPositionFactors.map((y) => HorizontalLine(
        startColomn: i, endColumn: i + 1, yPositionFactor: y,
      )));
    }
      ...
      while (currentY > 0) {
        final availableLines = _horizontalLines.where((line) {
          ...
          return lineY < currentY &&
            ...
            (b.yPositionFactor * _kCanvasHeight).compareTo(a.yPositionFactor * _kCanvasHeight));
      ...
      body: LayoutBuilder(builder: (context, constraints) => SingleChildScrollView(
        child: Center(child: ConstrainedBox(
          ...
          child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            if (isShowButton) ElevatedButton(
              onPressed: () {
                _startAnimation();
...
class HorizontalLine {
  const HorizontalLine({required this.startColomn, required this.endColumn, required this.yPositionFactor});
...
class AmidaPainter extends CustomPainter {
  AmidaPainter({required this.horizontalLines, required this.nameList, required this.lotteryList,
    required this.winningLinePaths, required this.image, required this.animationProgress});
  ...
  void paint(Canvas canvas, Size size) {
    final paint = Paint()..color = Colors.brown..strokeWidth = 4..style = PaintingStyle.stroke;
    for (var i = 0; i < horizontalLines.length / _kMaxHorizontalLinesPerColumn + 1; i++) {
      ...
      if (i < nameList.length) {
        ...
        nameTextPainter.paint(canvas, Offset(x - nameTextPainter.width / 2, -nameTextPainter.height - 5));
      }
      if (i < lotteryList.length && lotteryList[i] == AmidaLottery.win && image != null) {
        ...
        canvas.drawImageRect(image!, Rect.fromLTWH(0, 0, width, height),
          Rect.fromLTWH(x - drawWidth / 2, size.height + 5, drawWidth, drawHeight), Paint());
      }
    }
    ...
    for (var pathIndex = 0; pathIndex < pathCount; pathIndex++) {
      ...
      final pathProgress = ((animationProgress - startProgress) /
              (endProgress - startProgress)).clamp(0.0, 1.0);
      _paintPath(canvas, winningLinePaths[pathIndex], pathProgress,
  ...
  void _paintPath(Canvas canvas, List<Offset> path, double progress, Color color) {
    ...
    final linePaint = Paint()..color = color..strokeWidth = 10..style = PaintingStyle.stroke;
    ...
    if (completedSegments < segmentCount) {
      ...
      canvas.drawLine(start, Offset(start.dx + (end.dx - start.dx) * t,
          start.dy + (end.dy - start.dy) * t), linePaint);
    }
  }
